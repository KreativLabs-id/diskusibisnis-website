...
const NotificationItem: React.FC<NotificationItemProps> = ({ notification, onMarkAsRead }) => {
  ...
  const content = (
    <div
      className={`p-4 sm:p-5 hover:bg-slate-50 transition-all cursor-pointer border-b border-slate-100 last:border-0 ${notification.is_read
        ? 'bg-white'
        : 'bg-emerald-50/40'
        }`}
      onClick={handleClick}
      ...
      <div className="flex gap-4">
        ...
        <div className="flex-1 min-w-0 pt-1">
          <div className="flex items-start justify-between gap-2 mb-1">
            <h3 className={`text-sm sm:text-base font-semibold leading-snug ${notification.is_read ? 'text-slate-700' : 'text-slate-900'
              }`}>
              {notification.title}
            </h3>
      ...
      <div className="sticky top-0 z-30 bg-white border-b border-slate-200 px-4 py-3 sm:hidden flex items-center justify-between shadow-sm">
        ...
        {unreadCount > 0 && (
          <button
            ...
            className="text-xs font-medium text-emerald-600 px-2 py-1 bg-emerald-50 rounded-lg"
          >
            Tandai dibaca
          </button>
        )}
      ...
      <div className="w-full max-w-3xl mx-auto sm:px-4 sm:py-8">
        ...
        <div className="hidden sm:flex items-center justify-between mb-6 px-2">
          ...
          {unreadCount > 0 && (
            <button
              onClick={handleMarkAllAsRead}
              className="flex items-center gap-2 px-4 py-2 text-emerald-600 hover:text-emerald-700 hover:bg-emerald-50 rounded-xl transition-colors font-medium text-sm"
            >
        ...
        {/* Notifications List */}
        <div className="bg-white sm:rounded-2xl shadow-sm border-b sm:border border-slate-200 overflow-hidden min-h-[50vh]">
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-20 px-4 text-center">
              ...
              <h3 className="text-lg font-semibold text-slate-900 mb-2">Belum ada notifikasi</h3>
        ...
        {notifications.length > 0 && notifications.length >= 50 && (
          <div className="mt-6 text-center px-4">
            <button className="w-full sm:w-auto px-6 py-3 bg-white border border-slate-200 text-slate-600 rounded-xl hover:bg-slate-50 transition-colors font-medium text-sm shadow-sm">
              Muat lebih banyak
            </button>
          </div>
        )}
