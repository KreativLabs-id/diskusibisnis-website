...
Terima kasih telah menghubungi tim support DiskusiBisnis.
...
Terima kasih telah menghubungi kami. Tiket Anda dengan nomor {tiket} telah kami terima dan sedang dalam proses penanganan.

Tim kami akan segera meninjau dan memberikan solusi terbaik untuk Anda.

Salam,
Tim Support DiskusiBisnis`
  },
  {
    id: 'need_info',
...
Untuk dapat membantu menyelesaikan masalah Anda, kami memerlukan informasi tambahan berikut:
1. 
2. 
3. 

Mohon balas email ini dengan informasi yang diminta.
...
Terima kasih telah melaporkan masalah akun Anda.

Kami telah memeriksa akun Anda dan [jelaskan tindakan yang diambil].

Silakan coba login kembali dan pastikan:
- Menggunakan email yang terdaftar
- Password sudah benar (perhatikan huruf besar/kecil)
- Jika lupa password, gunakan fitur "Lupa Password"

Jika masih mengalami kendala, silakan hubungi kami kembali.
...
Terima kasih telah melaporkan bug ini kepada kami.

Kami telah mencatat laporan Anda dan tim teknis kami sedang menginvestigasi masalah tersebut. Kami akan memberikan update setelah masalah berhasil diperbaiki.

Kami sangat menghargai kontribusi Anda dalam membantu meningkatkan kualitas platform DiskusiBisnis.
...
Terima kasih atas saran fitur yang Anda berikan!

Kami sangat menghargai masukan dari pengguna seperti Anda. Saran Anda telah kami catat dan akan dipertimbangkan untuk pengembangan platform ke depannya.

Terus ikuti update dari DiskusiBisnis untuk melihat fitur-fitur baru yang akan kami hadirkan.
...
Tim Support DiskusiBisnis`
  ...
  {
    id: 'resolved',
    label: '✅ Masalah Selesai',
    text: `Halo {nama},

Kami informasikan bahwa masalah yang Anda laporkan telah berhasil diselesaikan.

[Jelaskan solusi/tindakan yang diambil]

Jika Anda masih mengalami kendala atau memiliki pertanyaan lain, jangan ragu untuk menghubungi kami kembali.

Terima kasih telah menggunakan DiskusiBisnis!
...
Jika tidak ada, kami akan menutup tiket ini. Anda dapat membuka tiket baru kapan saja jika memerlukan bantuan.
...
export default function AdminSupportPage() {
  ...
  // Insert template ke textarea
  const insertTemplate = (template: typeof replyTemplates[0]) => {
    if (!selectedTicket) return;
    
    let text = template.text
      .replace(/{nama}/g, selectedTicket.name)
      .replace(/{tiket}/g, selectedTicket.ticket_number)
      .replace(/{email}/g, selectedTicket.email)
      .replace(/{subjek}/g, selectedTicket.subject);
    
    // Append ke existing message atau replace
              ...
              tickets.map((ticket) => (
                ...
                >
                  ...
                  <div className="flex items-center gap-2 mt-1.5">
                    <span className={`text-xs px-2 py-0.5 rounded-full ${categoryColors[ticket.category] || categoryColors.other}`}>
              ...
              <div className="p-4 border-b">
                <div className="flex items-start justify-between">
                  <div>
                    ...
                    <div className="flex items-center gap-2 mt-1.5">
                      <span className={`text-xs px-2.5 py-1 rounded-full font-medium ${categoryColors[selectedTicket.category] || categoryColors.other}`}>
                        {categoryIcons[selectedTicket.category] || '📝'} {categoryLabels[selectedTicket.category] || selectedTicket.category}
                      </span>
                    </div>
              ...
              <div className="p-4 border-t">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <div className="relative">
                    ...
                    {/* Template Dropdown */}
                    {showTemplates && (
                      <div className="absolute right-0 top-full mt-1 w-64 bg-white border rounded-xl shadow-lg z-10 py-2 max-h-80 overflow-y-auto">
                        <p className="px-3 py-1.5 text-xs font-semibold text-slate-500 border-b mb-1">Pilih Template</p>
